...
export class TokenServiceStub implements TokenService {
  ...
  async consume({ type, id }: ConsumeRequest): Promise<void> {
  ...
  async destroy({ type, id }: DestroyRequest): Promise<void> {
  ...
  async upsert({ type, id, expiresIn, payload: payloadBuffer }: UpsertRequest): Promise<void> {
  ...
  async revokeByGrantId({ grantId }: RevokeByGrantIdRequest): Promise<void> {
