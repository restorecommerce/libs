...
import { handleError } from './iam/repository';
...
export interface RestoreCommerceFeature<TState extends object = {}, TContent extends object = {}, TFacade extends object = {}> {
  key: string;
  handle(facade: Facade<RestoreCommerceFeature<TState, TContent>> & TFacade): Facade<RestoreCommerceFeature<TState, TContent>>;
};
...
export interface Facade<TFeature extends RestoreCommerceFeature = any> {
  ...
  supports<TFeature2 extends RestoreCommerceFeature>(feature: TFeature2): this is Facade<MergeFeatures<TFeature, TFeature2>>;
}
...
export class FacadImpl<TFeature extends RestoreCommerceFeature = any> implements Facade<TFeature> {
  ...
  feature<TNewFeature extends RestoreCommerceFeature<ExtractFacadeState<TFeature>, ExtractFacadeContext<TFeature>, {}>>(feature: TNewFeature): Facade<RestoreCommerceFeature<ExtractFacadeState<TFeature> & ExtractFacadeState<TNewFeature> & object, ExtractFacadeContext<TFeature> & ExtractFacadeContext<TNewFeature> & object, ExtractFacadeFacade<TFeature> & ExtractFacadeFacade<TNewFeature> & object>> & ExtractFacadeFacade<TNewFeature> {
    if (this.features.includes(feature.key)) {
      throw new Error('TODO');
    }
    this.features.push(feature.key);
    feature.handle(this as any);
    return this as any;
  }

  supports<TFeature extends RestoreCommerceFeature>(feature: TFeature): this is Facade<TFeature> {
    return this.features.includes(feature.key);
  }
