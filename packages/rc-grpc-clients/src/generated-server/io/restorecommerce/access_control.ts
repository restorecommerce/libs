...
export const Response = {
  encode(message: Response, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.operation_status !== undefined) {
      OperationStatus.encode(message.operation_status, writer.uint32(34).fork()).ldelim();
    }
    return writer;
  },
...
export const ReverseQuery = {
  ...
  toJSON(message: ReverseQuery): unknown {
    ...
    if (message.policy_sets) {
      obj.policy_sets = message.policy_sets.map((e) => e ? PolicySetRQ.toJSON(e) : undefined);
    } else {
      obj.policy_sets = [];
    }
    if (message.obligations) {
