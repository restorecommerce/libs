...
import { protoMetadata as protoMetadata14, Timestamp } from "../../google/protobuf/timestamp";
...
function createBaseInvoiceSection(): InvoiceSection {
  return { fulfillment_id: undefined, selected_parcels: [] };
}
...
export const InvoiceSection = {
  encode(message: InvoiceSection, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    for (const v of message.selected_parcels) {
      writer.uint32(18).string(v!);
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          ...
          message.selected_parcels.push(reader.string());
          continue;
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
  ...
  fromJSON(object: any): InvoiceSection {
    return {
      ...
      selected_parcels: Array.isArray(object?.selected_parcels)
        ? object.selected_parcels.map((e: any) => String(e))
        : [],
    };
  },
  ...
  toJSON(message: InvoiceSection): unknown {
    ...
    if (message.selected_parcels) {
      obj.selected_parcels = message.selected_parcels.map((e) => e);
    } else {
      obj.selected_parcels = [];
    }
    return obj;
  },
  ...
  fromPartial(object: DeepPartial<InvoiceSection>): InvoiceSection {
    ...
    message.selected_parcels = object.selected_parcels?.map((e) => e) || [];
    return message;
  },
};
